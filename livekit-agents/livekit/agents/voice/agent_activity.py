...
class AgentActivity(RecognitionHooks):
    ...
    def _try_interrupt_speech(self, reason: str = "") -> None:
        """Helper method to attempt interrupting current speech"""
        if self._current_speech is None:
            logger.debug(f"VAD event ignored: no current speech")
            return
            
        if self._current_speech.interrupted:
            logger.debug("VAD event ignored: current speech already interrupted")
            return
            
        if not self._current_speech.allow_interruptions:
            logger.debug("VAD event ignored: current speech does not allow interruptions")
            return

        logger.debug(f"Interrupting speech with VAD event: speech_id={self._current_speech.id}, reason={reason}")
        log_event(
        ...
        if self._rt_session is not None:
            logger.debug("Interrupting realtime session")
            self._rt_session.interrupt()

        self._current_speech.interrupt()
        logger.debug("Successfully interrupted current speech")

    def on_vad_inference_done(self, ev: vad.VADEvent) -> None:
        logger.debug(f"VAD event received: type={ev.type}, duration={ev.speech_duration}")
        
        if self._turn_detection_mode not in ("vad", None):
            logger.debug(f"VAD event ignored: turn_detection_mode={self._turn_detection_mode} not in ('vad', None)")
            return

        if isinstance(self.llm, llm.RealtimeModel) and self.llm.capabilities.turn_detection:
            logger.debug("VAD event ignored: using realtime model with turn detection")
            return

        # For START_OF_SPEECH events, try to interrupt immediately
        if ev.type == vad.VADEventType.START_OF_SPEECH:
            self._try_interrupt_speech("speech_started")
            return
            
        # For other events, check duration threshold
        if ev.speech_duration < self._session.options.min_interruption_duration:
            logger.debug(f"VAD event ignored: speech_duration={ev.speech_duration} < min_interruption_duration={self._session.options.min_interruption_duration}")
            return
        ...
        if (
            ...
            if word_count < self._session.options.min_interruption_words:
                logger.debug(f"VAD event ignored: word_count={word_count} < min_interruption_words={self._session.options.min_interruption_words}")
                return

        self._try_interrupt_speech("speech_detected")
    ...
    def on_interim_transcript(self, ev: stt.SpeechEvent) -> None:
        ...
        self._session.emit(
            "user_input_transcribed",
            UserInputTranscribedEvent(transcript=text, is_final=False),
        )
        
        # Interrupt current speech if allowed
        if self._current_speech and self._current_speech.allow_interruptions and self._session.options.allow_interruptions:
            self._try_interrupt_speech("interim_transcript")
